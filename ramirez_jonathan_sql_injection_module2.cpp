...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    //In this case, the 4 sql injections all contain the "or" string thus
    //This string will be used to check if there are any "or" in the passwords.
    std::string sqlStringOr = "or";
    //std::string dataFromDatabase = sql;
    if (sql.find(sqlStringOr) != -1) {
