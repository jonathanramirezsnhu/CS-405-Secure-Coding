...
std::string encrypt_decrypt(const std::string& source, const std::string& key)
{
    ...
    // our output length must equal the source length
...
std::string read_file(const std::string& filename)
{
    ...
    //empty string to put text from inputdatafile
    std::string file_text = "";
    std::string mstring;
    //opens the file so it can be read
    std::ifstream file_stream(filename);

    //adds the data from filename to the file_text string 
    while (std::getline(file_stream, mstring))
    {
        //everything in each line is added to the mstring and then to the file_text
        file_text = file_text + mstring + "\n";
    }
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
    ...
    //  Line 3: key used
    //  Line 4+: data
    //Gets time form the system and prints in the yyy-mm-dd format
    time_t rawtime;
    char buffer[80];
    time(&rawtime);
    //timeinfo = localtime(&rawtime);
    std::strftime(buffer, 80, "%Y-%m-%d", std::localtime(&rawtime)); //WAS NOT ABLE TO FIGURE THIS OUT

    //writes the data tot he file called fileStream
    std::ofstream fileStream(filename);
    fileStream << student_name << std::endl;
